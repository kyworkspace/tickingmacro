...
const excuteFunction = async (body) => {
  ...
  if (Number(startTime) % 2 === 0) {
    ...
    startTimeOptionValue =
      Number(startTime) - 1 < 10
        ? `0${Number(startTime) - 1}`
        : (Number(startTime) - 1).toString();
  }
  //도착시간 조정
      ...
      setTimeout(async () => {
        ...
        for (const [index, tr] of trs.entries()) {
          if (!result) {
            ...
            const timeText = await (
              await timeEle.getProperty("textContent")
            ).jsonValue();

            if (!timeCompare(timeText, "lower", endTime)) skip = true; //지정 시간을 초과한 경우에는 break

            if (
              timeCompare(timeText, "over", startTime) &&
              timeCompare(timeText, "lower", endTime)
            ) {
              //예약하기 버튼칸
              const aTag = await tds[6].$("a");
              const text = await (
                await aTag.getProperty("textContent")
              ).jsonValue();
              if (text === "예약하기") {
                result = true;
                aTag.click();
                //예약 시간
                const reservTime = await (
                  await (await tds[4].$(".time")).getProperty("textContent")
                ).jsonValue();
                setTimeout(async () => {
                  console.log(
                    `${userId}의 ${new Date().toLocaleDateString()} ${reservTime} 시간이 예약 되었습니다.`
                  );
                }, 1000);
                break;
              }
        ...
        if (!result) {
          ...
          const btn = await page.$(
            ".tal_r > input[type=button].btn_burgundy_dark"
          );
